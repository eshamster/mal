...
let repl_env = new Dictionary<string, MalType>()

let init () : _ =
  repl_env.Add("+", MakeNumberMulFunc (fun x y -> x + y))
  repl_env.Add("-", MakeNumberMulFunc (fun x y -> x - y))
  repl_env.Add("/", MakeNumberMulFunc (fun x y -> x / y))
  repl_env.Add("*", MakeNumberMulFunc (fun x y -> x * y))
  ...
  let eval_ast (ast:MalType) : MalType =
    match ast with
      | :? MalSymbol as s ->
          ...
          if repl_env.ContainsKey name then
            repl_env.[name]
          else
            failwith ("The symbol is not defined: " + name)
      | :? MalList as s ->
